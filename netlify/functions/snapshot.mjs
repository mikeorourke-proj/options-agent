...
export default async (request) => {
  ...
  try {
    ...
    // Levered ETF rebalance history across all dates
    const rebalHistory = url.searchParams.get("rebalance-history");
    if (rebalHistory) {
      let dateIndex = [];
      try { dateIndex = await store.get("date-index", { type: "json" }) || []; } catch { dateIndex = []; }
      const points = [];
      for (const dt of dateIndex.slice(-60)) {
        try {
          const snap = await store.get(`daily-${dt}`, { type: "json" });
          if (snap && snap.leveredRebalance) {
            const lr = snap.leveredRebalance;
            const point = { date: dt, capturedAt: lr.capturedAt, totalRebalance: lr.totalRebalance, byUnderlying: {} };
            if (lr.byUnderlying) {
              for (const [ul, data] of Object.entries(lr.byUnderlying)) {
                point.byUnderlying[ul] = {
                  deltaS: data.deltaS, regime: data.regime,
                  totalRebalance: data.totalRebalance, totalAum: data.totalAum,
                  funds: (data.funds || []).map(f => ({ ticker: f.ticker, x: f.x, gamma: f.gamma, aum: f.aum, rebalance: f.rebalance })),
                };
              }
            }
            points.push(point);
