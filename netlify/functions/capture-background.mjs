...
export default async (request) => {
  ...
  // ─── LEVERED ETF REBALANCE CAPTURE ──────────────────────────────
  if (allDone && mergedTickers.length > 10) {
    const LEVERED_ETFS = [
      { ticker: "TQQQ", underlying: "QQQ", x: 3, gamma: 6 },
      { ticker: "QLD",  underlying: "QQQ", x: 2, gamma: 2 },
      { ticker: "SQQQ", underlying: "QQQ", x: -3, gamma: 12 },
      { ticker: "UPRO", underlying: "SPY", x: 3, gamma: 6 },
      { ticker: "SPXL", underlying: "SPY", x: 3, gamma: 6 },
      { ticker: "SSO",  underlying: "SPY", x: 2, gamma: 2 },
      { ticker: "SPXS", underlying: "SPY", x: -3, gamma: 12 },
      { ticker: "SDS",  underlying: "SPY", x: -2, gamma: 6 },
      { ticker: "SH",   underlying: "SPY", x: -1, gamma: 2 },
      { ticker: "SOXL", underlying: "SMH", x: 3, gamma: 6 },
      { ticker: "SOXS", underlying: "SMH", x: -3, gamma: 12 },
      { ticker: "USD",  underlying: "SMH", x: 2, gamma: 2 },
      { ticker: "TNA",  underlying: "IWM", x: 3, gamma: 6 },
      { ticker: "TZA",  underlying: "IWM", x: -3, gamma: 12 },
      { ticker: "TECL", underlying: "XLK", x: 3, gamma: 6 },
      { ticker: "UVXY", underlying: "VIX", x: 1.5, gamma: 0.75 },
    ];

    // Get underlying daily % changes from already-captured snapshot data
    const underlyingChanges = {};
    for (const t of mergedTickers) {
      if (t.changePerc !== undefined && t.changePerc !== 0) underlyingChanges[t.ticker] = t.changePerc;
    }

    // Get GEX regimes from captured data
    const regimes = {};
    for (const t of mergedTickers) { if (t.regime) regimes[t.ticker] = t.regime; }

    // Fetch AUM (market_cap) for each levered ETF from Polygon reference
    const rebalFunds = [];
    for (let i = 0; i < LEVERED_ETFS.length; i++) {
      const etf = LEVERED_ETFS[i];
      let aum = 0, price = 0, etfChangePerc = 0;
      try {
        const refData = await polyFetch(`https://api.polygon.io/v3/reference/tickers/${etf.ticker}?apiKey=${apiKey}`);
        if (refData && refData.results && refData.results.market_cap > 0) {
          aum = refData.results.market_cap;
        }
      } catch {}
      // Also get ETF's own price/change
      try {
        const snap = await polyFetch(`https://api.polygon.io/v2/snapshot/locale/us/markets/stocks/tickers/${etf.ticker}?apiKey=${apiKey}`);
        if (snap && snap.ticker) {
          price = snap.ticker.lastTrade?.p || snap.ticker.day?.c || 0;
          etfChangePerc = snap.ticker.todaysChangePerc || 0;
        }
      } catch {}

      const deltaS = (underlyingChanges[etf.underlying] || 0) / 100;
      const rebalance = aum > 0 ? etf.gamma * deltaS * aum : 0;
      rebalFunds.push({ ticker: etf.ticker, underlying: etf.underlying, x: etf.x, gamma: etf.gamma, aum, price, etfChangePerc, rebalance });

      if (i < LEVERED_ETFS.length - 1) await sleep(150);
    }

    // Aggregate by underlying
    const byUnderlying = {};
    for (const f of rebalFunds) {
      if (!byUnderlying[f.underlying]) byUnderlying[f.underlying] = { underlying: f.underlying, deltaS: (underlyingChanges[f.underlying] || 0) / 100, regime: regimes[f.underlying] || null, totalRebalance: 0, totalAum: 0, funds: [] };
      byUnderlying[f.underlying].totalRebalance += f.rebalance;
      byUnderlying[f.underlying].totalAum += f.aum;
      byUnderlying[f.underlying].funds.push(f);
    }

    const totalRebalAll = Object.values(byUnderlying).reduce((s, g) => s + g.totalRebalance, 0);

    snapshot.leveredRebalance = {
      capturedAt: new Date().toISOString(),
      totalRebalance: totalRebalAll,
      byUnderlying,
      funds: rebalFunds,
    };

    console.log(`  Levered ETF rebalance: ${rebalFunds.filter(f => f.aum > 0).length}/${rebalFunds.length} funds with AUM, total rebal: $${(totalRebalAll / 1e6).toFixed(0)}M`);
  }
