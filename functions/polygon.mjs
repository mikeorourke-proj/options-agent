export default async (request) => {
  ...
  switch (endpoint) {
    ...
    case "snapshot":
      ...
      break;
    case "reference": {
      // Ticker details — returns market_cap, share_class_shares_outstanding, weighted_shares_outstanding
      const refR = await fetch(`https://api.polygon.io/v3/reference/tickers/${ticker}?apiKey=${apiKey}`, { headers: { "Accept": "application/json" } });
      const refD = await refR.json();
      if (refD && refD.results) {
        return new Response(JSON.stringify({
          ticker: refD.results.ticker,
          name: refD.results.name,
          market_cap: refD.results.market_cap || 0,
          share_class_shares_outstanding: refD.results.share_class_shares_outstanding || 0,
          weighted_shares_outstanding: refD.results.weighted_shares_outstanding || 0,
        }), { headers: { ...headers, "Cache-Control": "public, max-age=3600" } });
      }
      return new Response(JSON.stringify({ error: "no reference data" }), { status: 404, headers });
    }
    default:
